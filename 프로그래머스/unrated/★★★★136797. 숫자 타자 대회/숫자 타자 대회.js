...
// 벽느꼈다
// 플로이드 + (우선순위큐 or BFS)
// 플로이드는 정점이 별로 없고 모든 정점으로의 최소 경로를 구해야할 때 사용
// 처음에 DP로 풀다가 도저히 안되서 포기했다. 오른손, 왼손 경로가 똑같을때 어떻게 해야하는지를 모르겠다
// 결국엔 Heap으로 발길을 돌렸지만 시간초과에 걸렸다.
// 뭐때문에 시간초과가 걸린지 한참을 헤맸다. 별 생각을 다했는데 결국엔 이 문제는 완전탐색이다. 무조건 오른손, 왼손 경우의 수를 다 따져야 한다.
// 혹시 그리디인가, 아니면 추가적으로 걸러야하는 경우의 수가 있는가를 한참을 헤맸는데 답은 생각보다 간단했다.
// 같은 인덱스 번호의 손 위치 L R 에서 가중치 W 보다 크거나 같은 값은 걸러야한다. 처음에는 이게 왜 중복인지 몰랐다..
// n개의 번호만큼 반복하기 때문에 한번의 위치 L R에서 2개의 경우의 수로 나눠지기 떄문에 중복 제거를 안하면 2^n만큼 쌓인다.
// 그리고 L R 과 R L 도 중복이다. BFS 코드도 올렸다
