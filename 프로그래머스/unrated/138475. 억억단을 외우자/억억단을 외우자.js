...
class MaxHeap {
    ...
    heapify(n, i) {
        ...
        if (l <= n){
            if(this.heap[l].value > this.heap[largest].value)
                largest = l;
            else if(this.heap[l].value === this.heap[largest].value && this.heap[l].index < this.heap[largest].index)
                largest = l;
        } 
        //오른쪽 자식이 더 크면 현재 노드와 바꿔준다
        if (r <= n){
            if(this.heap[r].value > this.heap[largest].value)
                largest = r;
            else if(this.heap[r].value === this.heap[largest].value && this.heap[r].index < this.heap[largest].index)
                largest = r;
        }
        ...
        insert(x) {
          ...
          while (i > 1 && ((this.heap[parent].value < this.heap[i].value) || (this.heap[parent].value === this.heap[i].value && this.heap[parent].index > this.heap[i].index))) {
